...
export default function Page() {
  ...
  const [soundOn, setSoundOn] = useState<boolean>(() => {
    ...
    const v = localStorage.getItem('ap_sound_enabled');
    return v ? v === '1' : true;
  });
  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('ap_sound_enabled', soundOn ? '1' : '0');
    }
  }, [soundOn]);
  ...
  useEffect(() => {
    ...
    const ch = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'infusions', filter: `user_id=eq.${user.id}` }, () => load())
      .subscribe();
    ...
    try {
      const res = await fetch('/api/notify', {
        ...
        body: JSON.stringify({
          ...
          infusionId: inf.id,
          to: recipient || null,
          data: { patient: inf.patient_name, room: inf.room, bed: inf.bed, end_time: inf.end_time }
        })
      });
      return res.ok;
    } catch {
  ...
  return (
    <div className="page">
        ...
        <div className="card-head">
          ...
          <button
            ...
            onClick={async () => {
              ...
              await supabase.from('infusions').delete().eq('user_id', user.id).lte('end_time', nowISO);
            }}
      ...
      <style jsx>{`
        :root{--blue:#1d4ed8;--blue-500:#2563eb;--green:#22c55e;--yellow:#f59e0b;--red:#ef4444;--bg:#f7fafc;--card:#fff;--line:#e5e7eb;--muted:#6b7280;--text:#111827}
        .page{max-width:980px;margin:0 auto;padding:16px 14px 28px;background:var(--bg);min-height:100dvh;color:var(--text)}
        ...
        .sub{font-size:13px;color:#374151;opacity:.9;margin-top:2px}
        ...
        .btn-outline{background:#fff;border:1px solid var(--line);padding:8px 12px;border-radius:10px;font-weight:700;color:var(--blue)}
        .btn-danger{background:#fff;border:1px solid var(--line);padding:8px 12px;border-radius:10px;font-weight:700;color:var(--red)}
        .card{background:var(--card);border:1px solid var(--line);border-radius:14px;padding:14px;margin-top:12px;box-shadow:0 6px 24px rgba(0,0,0,.04)}
        .card-head{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:10px}
        h2{font-size:18px;margin:2px 2px 6px;display:inline-flex;align-items:center;gap:8px}
        .badge-soft{display:inline-block;font-size:12px;padding:6px 10px;border-radius:999px;background:#eff6ff;color:var(--blue);border:1px solid #dbeafe}
        .formcol{display:grid;grid-template-columns:1fr;gap:8px}
        label{font-size:13px;color:#374151}
        input,textarea{width:100%;background:#f9fafb;border:1px solid var(--line);padding:12px 12px;border-radius:10px;font-size:16px}
        textarea{min-height:120px}
        .row{display:flex;align-items:center;gap:12px;margin-top:8px;flex-wrap:wrap}
        .ck{display:inline-flex;gap:8px;align-items:center;font-size:14px}
        .btn{background:linear-gradient(90deg,var(--blue-500),var(--green));color:#fff;border:0;padding:12px 14px;border-radius:10px;font-weight:800;margin-left:auto;letter-spacing:.2px}
        .endhint{margin-top:6px;font-size:12px;color:var(--muted)}
        .list{display:grid;gap:8px}
        .rowitem{display:grid;grid-template-columns:1.2fr .9fr 1fr .9fr auto;gap:10px;align-items:center;padding:10px;border:1px solid var(--line);border-radius:12px;background:#fff}
        .cell .label{font-size:11px;color:var(--muted)}
        .cell .value{font-size:15px;font-weight:600}
        .count{display:flex;justify-content:flex-end}
        ...
        .foot{text-align:center;color:#6b7280;font-size:12px;margin-top:16px}
        @media(max-width:719px){.rowitem{grid-template-columns:1fr 1fr;}.count{grid-column:1/-1;justify-content:flex-start}}
      `}</style>
    </div>
  );
}
